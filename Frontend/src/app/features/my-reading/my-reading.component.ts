          ...
          @if (recentlyRead()) {
            ...
            <div class="active-book-banner-clean">
              <div class="banner-content">
                <img
                  [src]="recentlyRead()!.cover"
                  [alt]="recentlyRead()!.title"
                  class="active-cover"
                />
                ...
                <div class="active-info-card">
                  ...
                  <div class="progress-section">
                    <div class="progress-header">
                      <span
                        >Chapter {{ recentlyRead()!.currentChapter }} of
                        {{ recentlyRead()!.totalChapters }}</span
                      >
                      <span class="percentage"
                        >{{ recentlyRead()!.progressPercentage }}%</span
                      >
                    </div>
                    <div class="progress-bar">
                      <div
                        class="progress-fill"
                        [style.width.%]="recentlyRead()!.progressPercentage"
                      ></div>
                    </div>
                  </div>
                  ...
                  <button
                    class="btn-continue"
                    [routerLink]="['/read', recentlyRead()!.storyId]"
                  >
                    <svg
                      width="14"
                      height="14"
...
export class MyReadingComponent implements OnInit {
  ...
  ngOnInit() {
    this.authService
      ...
      .subscribe({
        next: (progressData) => {
          const history: ReadingHistory[] = progressData
            .map((p: any) => ({
              id: p._id,
              storyId: p.book?._id,
              ...
              lastReadDate: new Date(p.lastReadAt).toLocaleDateString(
                undefined,

...
@Component({
      ...
      <main class="chapter-editor">
        <header class="editor-header">
          ...
          <div class="actions" style="display: flex; gap: 12px; align-items: center;">
            ...
            <button class="btn-secondary" (click)="toggleTamilTyping()" [class.active-btn]="isTamilTypingEnabled" title="Toggle Tamil Keyboard" style="display: flex; gap: 6px; align-items: center;">
              <span style="font-weight: bold;">{{ isTamilTypingEnabled ? 'அ' : 'A' }}</span> 
            </button>
            <button class="btn-secondary" (click)="togglePreview()" [class.active-btn]="isPreviewMode" title="Preview Chapter">
        ...
        <div class="writing-workspace">
          ...
          <div class="editor-toolbar" [class.hidden]="isPreviewMode">
            <button (click)="execCommand('bold')" title="Bold (Ctrl+B)"><b>B</b></button>
            <button (click)="execCommand('italic')" title="Italic (Ctrl+I)"><i>I</i></button>
            <button (click)="execCommand('underline')" title="Underline (Ctrl+U)"><u>U</u></button>
            ...
            <button (click)="execCommand('insertHorizontalRule')" title="Divider">—</button>
            <button (click)="inlineImageInput.click()" title="Insert Image">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="3" width="18" height="18" rx="2" ry="2"></rect><circle cx="8.5" cy="8.5" r="1.5"></circle><polyline points="21 15 16 10 5 21"></polyline></svg>
            </button>
            <input type="file" #inlineImageInput hidden accept="image/*" (change)="uploadInlineImage($event)">
          </div>
  ...
  styles: [`
    ...
    .chapter-editor { flex: 1; display: flex; flex-direction: column; background: var(--paper-warm); position: relative; }
    ...
    .btn-secondary { background: transparent; border: 1px solid var(--border); padding: 8px 16px; border-radius: 100px; font-size: 13px; cursor: pointer; color: var(--ink); display: flex; align-items: center; justify-content: center; }
    ...
    .writing-workspace { flex: 1; padding: 32px 48px 80px; display: flex; flex-direction: column; gap: 16px; overflow-y: auto; max-width: 800px; margin: 0 auto; width: 100%; position: relative; }
    
    .chapter-title-input { font-family: var(--display); font-size: 32px; font-weight: 700; color: var(--ink); border: none; background: transparent; outline: none; margin-bottom: 8px; }
    ...
    .editor-toolbar button:hover { background: var(--paper-soft); }
    .editor-toolbar .divider { width: 1px; height: 20px; background: var(--border-soft); margin: 0 4px; }
    ...
    .content-editable-editor h1, .content-editable-editor h2 { margin-top: 24px; margin-bottom: 12px; font-family: var(--display); }
    .content-editable-editor blockquote { border-left: 4px solid var(--forest); margin: 16px 0; padding-left: 16px; color: var(--ink-soft); font-style: italic; }
    .content-editable-editor hr { border: none; border-top: 1px solid var(--border); margin: 24px 0; }
    .content-editable-editor img { max-width: 100%; border-radius: 8px; margin: 16px 0; display: block; }
    
    .content-editable-editor.preview-mode { cursor: default; }
    ...
    @media (max-width: 900px) { .editor-layout { flex-direction: column; } .meta-sidebar { width: 100%; height: auto; max-height: 300px; } .writing-workspace { padding: 24px; } .editor-footer { right: 24px; bottom: 12px; } }
  `]
})
    ...
    if (event.key === ' ' || event.key === 'Enter') {
      ...
      if (textNode.nodeType === Node.TEXT_NODE) {
        ...
        if (match) {
          ...
          const newText = textNode.textContent?.substring(0, range.startOffset - englishWord.length) 
                        + tamilWord 
          ...
          const newCursorPos = range.startOffset - englishWord.length + tamilWord.length;

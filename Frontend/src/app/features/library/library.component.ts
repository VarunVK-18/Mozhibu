...
type LibraryTab = 'bookmarks' | 'downloaded' | 'history' | 'completed' | 'following' | 'favorites' | 'collections';

@Component({
  selector: 'app-library',
  standalone: true,
  imports: [CommonModule, FormsModule, StoryCardComponent, UserCardComponent, RouterModule, TranslatePipe],
  template: `
      ...
      <div class="wrap library-content">
        ...
        <div class="controls-bar">
          <div class="search-box">
            ...
            <input type="text" [(ngModel)]="searchQuery" (ngModelChange)="applyFilters()" placeholder="Search library..." />
          </div>
        ...
        <div class="tabs-container">
          <div class="tabs">
            <button class="tab-btn" [class.active]="activeTab() === 'bookmarks'" (click)="activeTab.set('bookmarks')">Bookmarks</button>
            <button class="tab-btn" [class.active]="activeTab() === 'downloaded'" (click)="activeTab.set('downloaded')">Downloaded</button>
            <button class="tab-btn" [class.active]="activeTab() === 'history'" (click)="activeTab.set('history')">Reading History</button>
            <button class="tab-btn" [class.active]="activeTab() === 'completed'" (click)="activeTab.set('completed')">Completed</button>
            <button class="tab-btn" [class.active]="activeTab() === 'following'" (click)="activeTab.set('following')">Following</button>
            <button class="tab-btn" [class.active]="activeTab() === 'favorites'" (click)="activeTab.set('favorites')">Favorites</button>
            <button class="tab-btn" [class.active]="activeTab() === 'collections'" (click)="activeTab.set('collections')">Collections</button>
          </div>
        </div>
        ...
        <div class="tab-content" [ngSwitch]="activeTab()">
          ...
          <div *ngSwitchCase="'bookmarks'">
            <ng-container *ngTemplateOutlet="storyGrid; context: { list: filteredBookmarks }"></ng-container>
          </div>
            ...
            @if (filteredFollowing.length > 0) {
              ...
              <div class="empty-state">
                <div class="empty-icon">
                  <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M12 20h9"></path><path d="M16.5 3.5a2.121 2.121 0 0 1 3 3L7 19l-4 1 1-4L16.5 3.5z"></path></svg>
                </div>
          ...
          <div *ngSwitchCase="'favorites'">
            <ng-container *ngTemplateOutlet="storyGrid; context: { list: [] }"></ng-container>
          </div>
          ...
          <div *ngSwitchCase="'collections'">
            <div class="empty-state">
              ...
              <p>Create playlists of your favorite stories to organize your library.</p>
              <button class="btn-primary" (click)="alert('Collections coming soon!')">Create Collection</button>
            </div>
          </div>
        </div>
      </div>
  ...
  styles: [`
    ...
    .filter-group select {
      ...
      border: 1px solid var(--border);
      border-radius: 100px;
      background: var(--card) url('data:image/svg+xml;utf8,<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="black" stroke-width="2"><polyline points="6 9 12 15 18 9"></polyline></svg>') no-repeat right 12px center;
      appearance: none;
      font-size: 14px;
      font-family: var(--sans);
      cursor: pointer;
...
export class LibraryComponent implements OnInit {
  ...
  ngOnInit() {
    this.route.queryParams.subscribe(params => {
      if (params['tab']) {
        ...
        if (['bookmarks', 'downloaded', 'history', 'completed', 'following', 'favorites', 'collections'].includes(tab)) {
  ...
  loadData() {
    ...
    const storedDownloads = JSON.parse(localStorage.getItem('downloaded_books') || '[]');
    this.allDownloaded = storedDownloads.map((b: any) => ({
      id: b.id,
      title: b.title || 'Unknown Title',
      author: typeof b.author === 'object' ? b.author?.username : (b.author || 'Unknown Author'),
    ...
    const filterAndSort = (stories: Story[], requireComplete = false) => {
      ...
      if (q) {
        filtered = filtered.filter(s => 
          ...
          (typeof s.author === 'string' && s.author.toLowerCase().includes(q))
        );
      }

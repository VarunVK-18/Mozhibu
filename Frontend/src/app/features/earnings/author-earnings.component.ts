...
@Component({
        ...
        @if (isLoading()) {
          ...
          <div class="table-section">
            ...
            @if (earnings().length === 0) {
              <div class="empty-state">
                ...
                <p>No earnings yet. Your first earnings will appear here after the end of the month.</p>
              </div>
            } @else {
              <div class="data-table">
                ...
                @for (e of earnings(); track e._id) {
                  <div class="table-row">
                    <span class="period">{{ monthName(e.month) }} {{ e.year }}</span>
                    <span class="reads">{{ e.qualifiedReads.toLocaleString() }}</span>
          ...
          <div class="how-it-works">
            ...
            <div class="formula-card">
              <code>Your Earnings = (Your Qualified Reads ÷ Total Platform Reads) × Authors Pool</code>
            </div>
            <p>The Authors Pool is <strong>15%</strong> of monthly net ad revenue. Qualified reads require ≥30% chapter completion and ≥60 seconds reading time.</p>
          </div>
        }
      </div>
    </div>
  `,
  styles: [`
    .earnings-page { min-height: 100vh; background: #0d0d0d; color: #f5f0e8; padding-bottom: 80px; }
    ...
    .note-card { background: #1a1a2a; border-left: 3px solid #6366f1; padding: 14px 18px; border-radius: 8px; font-size: 13px; color: #9ca3af; margin-bottom: 40px; }

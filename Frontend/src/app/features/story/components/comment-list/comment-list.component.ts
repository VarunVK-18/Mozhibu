          ...
          <div class="avg-score">
            ...
            <div class="stars-display">
              @for (star of [1,2,3,4,5]; track star) {
                <svg viewBox="0 0 24 24" [attr.fill]="star <= roundedAverageRating ? 'currentColor' : 'none'" [attr.stroke]="star <= roundedAverageRating ? 'none' : 'currentColor'" stroke-width="2" class="avg-star-icon">
          ...
          <div class="rating-bars">
            @for (dist of ratingDistribution; track dist.stars) {
              <div class="bar-row">
                ...
                <svg class="mini-star-icon" viewBox="0 0 24 24" fill="currentColor"><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"></path></svg>
                <div class="progress-bar-bg">
                  <div class="progress-bar-fill" [style.width.%]="dist.percent"></div>
                </div>
              </div>
            }
          </div>
            ...
            <div class="input-footer">
              <div class="emoji-picker">
                @for (emoji of quickEmojis; track emoji) {
                  <button class="emoji-btn" (click)="addEmojiToComment(emoji)" type="button">{{ emoji }}</button>
                }
              </div>
              ...
              <div class="comment-content">
                <div class="comment-header">
                  ...
                  <div class="comment-menu-wrapper">
                    <button class="btn-icon menu-btn" (click)="toggleMenu(comment.id, $event)">
                    ...
                    <div class="comment-dropdown" [class.open]="activeDropdownId === comment.id">
                      <button *ngIf="isStoryAuthor" class="dropdown-item" (click)="pinComment(comment)">
                      ...
                      <button *ngIf="comment.authorName === currentUserName" class="dropdown-item delete" (click)="deleteComment(comment.id)">
                      ...
                      <button *ngIf="comment.authorName !== currentUserName" class="dropdown-item" (click)="reportComment(comment.id)">
                ...
                <div class="comment-actions">
                  ...
                  <button class="btn-text" (click)="openReplyBox(comment.id, comment.id)">Reply</button>
                </div>
                ...
                <div class="comment-input-area reply-area" *ngIf="activeReplyId === comment.id">
                  ...
                  <div class="input-wrapper">
                    <textarea [(ngModel)]="replyText" placeholder="Add a reply..." rows="1" #replyInputRef></textarea>
                    
                    <div class="input-footer" *ngIf="replyText.trim().length > 0">
                      <div class="emoji-picker">
                        @for (emoji of quickEmojis; track emoji) {
                          <button class="emoji-btn" (click)="addEmojiToReply(emoji)" type="button">{{ emoji }}</button>
                        }
                      </div>
                      <div class="input-actions">
                    ...
                    <div class="comment-card reply-card">
                      ...
                      <div class="comment-content">
                        <div class="comment-header">
                          ...
                          <div class="comment-menu-wrapper">
                            <button class="btn-icon menu-btn" (click)="toggleMenu(reply.id, $event)">
                              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="1"></circle><circle cx="12" cy="5" r="1"></circle><circle cx="12" cy="19" r="1"></circle></svg>
                            </button>
                            <div class="comment-dropdown" [class.open]="activeDropdownId === reply.id">
                              <button *ngIf="reply.authorName === currentUserName" class="dropdown-item delete" (click)="deleteComment(reply.id)">
                                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M3 6h18"></path><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path></svg>
                                Delete
                              </button>
                              <button *ngIf="reply.authorName !== currentUserName" class="dropdown-item" (click)="reportComment(reply.id)">
                                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M4 15s1-1 4-1 5 2 8 2 4-1 4-1V3s-1 1-4 1-5-2-8-2-4 1-4 1z"></path><line x1="4" y1="22" x2="4" y2="15"></line></svg>
                                Report
                              </button>
                            </div>
                        ...
                        <p class="comment-text">
                          <span class="mentioned-user" *ngIf="reply.text && reply.text.startsWith('@')">{{ reply.text.split(' ')[0] }}</span> 
                          {{ reply.text && reply.text.startsWith('@') ? reply.text.substring(reply.text.indexOf(' ') + 1) : reply.text }}
                        </p>
                        
                        <!-- Reply actions -->
                        <div class="comment-actions">
                          <button class="btn-icon" [class.active]="reply.isLiked" (click)="toggleLike(reply.id)">
                            <svg viewBox="0 0 24 24" fill="currentColor" *ngIf="reply.isLiked" stroke="none"><path d="M14 9V5a3 3 0 0 0-3-3l-4 9v11h11.28a2 2 0 0 0 2-1.7l1.38-9a2 2 0 0 0-2-2.3zM7 22H4a2 2 0 0 1-2-2v-7a2 2 0 0 1 2-2h3"></path></svg>
                            <svg viewBox="0 0 24 24" fill="none" *ngIf="!reply.isLiked" stroke="currentColor" stroke-width="2"><path d="M14 9V5a3 3 0 0 0-3-3l-4 9v11h11.28a2 2 0 0 0 2-1.7l1.38-9a2 2 0 0 0-2-2.3zM7 22H4a2 2 0 0 1-2-2v-7a2 2 0 0 1 2-2h3"></path></svg>
                            <span>{{ reply.likes }}</span>
                          </button>
                          <button class="btn-icon" [class.active]="reply.isDisliked" (click)="toggleDislike(reply.id)">
                            <svg viewBox="0 0 24 24" fill="currentColor" *ngIf="reply.isDisliked" stroke="none"><path d="M10 15v4a3 3 0 0 0 3 3l4-9V2H5.72a2 2 0 0 0-2 1.7l-1.38 9a2 2 0 0 0 2 2.3zm7-13h2.67A2.31 2.31 0 0 1 22 4v7a2.31 2.31 0 0 1-2.33 2H17"></path></svg>
                            <svg viewBox="0 0 24 24" fill="none" *ngIf="!reply.isDisliked" stroke="currentColor" stroke-width="2"><path d="M10 15v4a3 3 0 0 0 3 3l4-9V2H5.72a2 2 0 0 0-2 1.7l-1.38 9a2 2 0 0 0 2 2.3zm7-13h2.67A2.31 2.31 0 0 1 22 4v7a2.31 2.31 0 0 1-2.33 2H17"></path></svg>
                          </button>
                          <button class="btn-text" (click)="openReplyBox(reply.id, comment.id, reply.authorName)">Reply</button>
                        </div>
                        
                        <!-- Reply Input Box (Under Nested Reply) -->
                        <div class="comment-input-area reply-area" *ngIf="activeReplyId === reply.id">
                          <img [src]="currentUserAvatar" alt="You" class="avatar avatar-sm">
                          <div class="input-wrapper">
...
export class CommentListComponent {
  ...
  get isStoryAuthor(): boolean {
    return !!this.storyAuthorName && this.currentUserName === this.storyAuthorName;
  }
  
  // Rating Dashboard Logic
  get ratedComments() {
    return this.comments.filter(c => typeof c.rating === 'number' && c.rating > 0);
  }
  ...
  toggleMenu(commentId: string, event: Event) {
    ...
    this.activeDropdownId = this.activeDropdownId === commentId ? null : commentId;
  }
  ...
  openReplyBox(replyToId: string, parentCommentId: string, authorName?: string) {

      ...
      <div class="left-panel">
        ...
        <div class="tagline-container">
          <a routerLink="/" class="logo" style="display: flex; flex-direction: row; align-items: center; gap: 12px; text-decoration: none; margin-bottom: 24px;">
            <img loading="lazy" src="assets/Mozhibu topbarcropednewLogo (1).png" alt="Mozhibu Logo" style="height: 48px; object-fit: contain;" />
    ...
    this.socialAuthService.authState.subscribe((user) => {
      ...
      if (user) {
        if (user.provider === 'GOOGLE' && user.idToken) {
          ...
          this.isLoading = true;
          this.auth.loginWithFacebook(user.authToken, this.signupForm.get('dob')?.value).subscribe({
            next: (res) => {
              console.log('Backend response:', res);
              this.isLoading = false;
              if (res.isNewUser || res.isProfileComplete === false) {
            ...
            error: (err) => {
              console.error('Backend Error:', err);
              this.isLoading = false;
              this.errorMessage = err.error?.msg || err.message || 'An error occurred during Facebook sign up.';
              alert('Error from backend: ' + this.errorMessage);
            },
          });
        } else {

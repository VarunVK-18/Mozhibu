...
@Component({
      ...
      <div class="auth-card">
        ...
        <div *ngIf="successMessage" class="success-msg">
          ...
          <div class="login-link" style="margin-top: 16px;">
            <a routerLink="/login" class="btn btn-primary" style="color: white; display: inline-block;">Go to Login</a>
          </div>
        </div>
        ...
        <form *ngIf="!successMessage" [formGroup]="resetForm" (ngSubmit)="onSubmit()">
          ...
          <div class="form-group">
            ...
            <div *ngIf="resetForm.get('password')?.touched && resetForm.get('password')?.hasError('pattern')" class="field-error">
          ...
          <div class="form-group">
            ...
            <div *ngIf="resetForm.hasError('passwordMismatch') && resetForm.get('confirmPassword')?.touched" class="field-error">
          ...
          <button type="submit" class="btn btn-primary" [disabled]="isLoading || resetForm.invalid || !token">
  ...
  `,
  styles: [`
    .auth-wrap { display: flex; justify-content: center; align-items: center; min-height: calc(100vh - 150px); background: var(--paper); padding: 40px 20px; }
    .auth-card { background: var(--card); padding: 40px 48px; border-radius: var(--radius-l); border: 1px solid var(--border-soft); box-shadow: 0 10px 30px -10px rgba(43, 38, 32, 0.1); max-width: 480px; width: 100%; }
    .auth-card h2 { margin-bottom: 8px; font-family: var(--display); font-size: 24px; text-align: center; }
    .auth-card p { color: var(--ink-soft); margin-bottom: 24px; font-size: 14px; text-align: center; }
    .form-group { margin-bottom: 20px; }
    .form-group label { display: block; margin-bottom: 6px; font-size: 13px; font-weight: 500; color: var(--ink); }
    .form-control { width: 100%; padding: 12px 14px; border: 1px solid var(--border-soft); border-radius: var(--radius-s); font-family: var(--body); font-size: 14px; transition: all 0.2s; }
    .form-control:focus { outline: none; border-color: var(--gold); }
    .input-wrapper { position: relative; display: flex; align-items: center; }
    .input-wrapper .form-control { padding-right: 40px; }
    .eye-btn { position: absolute; right: 12px; background: none; border: none; color: var(--ink-faint); cursor: pointer; display: flex; align-items: center; padding: 0; transition: color 0.2s; }
    .eye-btn:hover { color: var(--ink); }
    .field-error { color: var(--rose); font-size: 11px; margin-top: 6px; line-height: 1.3; display: block; }
    ...
    .success-msg { color: var(--forest); font-size: 13px; margin-bottom: 16px; text-align: center; background: var(--forest-tint); padding: 16px; border-radius: 4px; font-weight: 500;}
    .btn { width: 100%; }
...
export class ResetPasswordComponent implements OnInit {
  ...
  onSubmit() {
    ...
    this.auth.resetPassword(this.token, this.resetForm.value.password).subscribe({
      ...
      error: (err) => {
        this.isLoading = false;
        this.errorMessage = err.error?.msg || 'An error occurred. Please try again.';
      }
    });
  }
}

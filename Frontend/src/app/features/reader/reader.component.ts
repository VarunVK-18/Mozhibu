...
export class ReaderComponent implements OnInit, OnDestroy {
    ...
    effect(
      () => {
        ...
        if (episodes && episodes.length > 0) {
          const currentEp =
            episodes.find((e) => e.episode === this.currentChapterNum()) ||
            episodes[0];
          if (currentEp) {
            this.chapterTitle =
  ...
  processContent(html: string) {
    ...
    if (currentLang === 'en') {
      ...
      this.http
        .post<{ content: string }>(
          'http://localhost:5000/api/books/translate-html',
          {
            html: html,
        ...
        .subscribe({
          next: (res) => {
            this.chapterContent.set(
              this.sanitizer.bypassSecurityTrustHtml(res.content),
            );
  ...
  ngOnInit(): void {
    ...
    this.scrollSub = this.scrollSubject
  ...
  saveProgress(percentage: number) {
    if (this.storyId && this.authService.user()) {
      this.authService
        .updateReadingProgress(this.storyId, undefined, percentage)
        .subscribe({
  ...
  onLangChange(event: Event) {
    ...
    if (targetLang === 'en') {
      this.chapterContent.set(
        this.sanitizer.bypassSecurityTrustHtml(this.currentHtml),
      );
      return;
    }
    ...
    this.http
      ...
      .subscribe({
        ...
        error: (err) => {
          ...
          this.isTranslating.set(false);
          const fallbackHtml =
            `<div style="padding: 16px; margin-bottom: 24px; background: #FFF4E5; border-left: 4px solid #FFA000; color: #b27300; border-radius: 4px;">
          ...
          this.chapterContent.set(
            this.sanitizer.bypassSecurityTrustHtml(fallbackHtml),
          );
        },
      });
  }

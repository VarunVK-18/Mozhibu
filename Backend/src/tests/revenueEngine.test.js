 ...
 */
function computePools(netRevenueInPaise, platformPercent, authorsPercent, readersPercent) {
...
describe('Revenue Engine — Pool Splits', () => {
  it('should split ₹10,00,000 correctly at 80/15/5', () => {
    ...
    const { platformPool, authorsPool, readersPool } = computePools(R, 80, 15, 5);
    assert.strictEqual(authorsPool, 15_000_000,  'Authors pool should be ₹1,50,000 (15%)');
    assert.strictEqual(readersPool, 5_000_000,   'Readers pool should be ₹50,000 (5%)');
    assert.strictEqual(platformPool, 80_000_000, 'Platform pool should be ₹8,00,000 (80%)');
    assert.strictEqual(platformPool + authorsPool + readersPool, R, 'Pools must sum to R (no paise lost)');
  });
  ...
  it('should give platform the remainder (integer safety check)', () => {
    ...
    const { platformPool, authorsPool, readersPool } = computePools(R, 80, 15, 5);
    assert.strictEqual(platformPool + authorsPool + readersPool, R,
...
describe('Revenue Engine — Author Earnings (Spec Worked Example)', () => {
  it('Author A: (50,000 ÷ 5,00,000) × ₹1,50,000 = ₹15,000', () => {
    ...
    const earningsInPaise = computeAuthorEarnings(authorAReads, totalReads, authorsPool);
    const earningsInRupees = earningsInPaise / 100;

    assert.strictEqual(earningsInRupees, 15_000, `Expected ₹15,000 got ₹${earningsInRupees}`);
  });
  ...
  it('should handle large read counts without float overflow', () => {
    ...
    const earnings = computeAuthorEarnings(1_000_000, 10_000_000, 1_00_00_000 * 100);
...
describe('Revenue Engine — Reader Rewards (Spec Worked Example)', () => {
  it('Reader X: (2,000 ÷ 10,00,000) × ₹50,000 = ₹100', () => {
    ...
    const rewardInPaise = computeReaderReward(readerXScore, totalScore, readersPool);
    const rewardInRupees = rewardInPaise / 100;

    assert.strictEqual(rewardInRupees, 100, `Expected ₹100 got ₹${rewardInRupees}`);
  });

...
function generateValidWebhookSignature(body, secret) {
  const s =
    secret || process.env.RAZORPAY_WEBHOOK_SECRET || "webhook_test_secret";
...
describe("Razorpay Adapter — Key Configuration", () => {
  ...
  it("should have a non-empty Key Secret", () => {
    ...
    assert.ok(
      KEY_SECRET.length >= 16,
      "Key Secret should be at least 16 chars",
    );
    console.log(
      `   ✓ Key Secret: ${KEY_SECRET.slice(0, 4)}${"*".repeat(KEY_SECRET.length - 8)}${KEY_SECRET.slice(-4)}`,
    );
  });
});
...
describe("Razorpay Adapter — Payment Signature Verification", () => {
  ...
  it("should return FALSE for a completely wrong signature", () => {
    const result = adapter.verifyPayment(
      "order_abc",
      "pay_abc",
      "aaabbbccc000",
    );
...
describe("Razorpay Adapter — Webhook Signature Verification", () => {
  ...
  it("should reject a tampered webhook body", () => {
    ...
    assert.strictEqual(
      result,
      false,
      "Tampered body should fail webhook verification",
    );
  });
...
describe("Razorpay Adapter — Order Creation (Mocked)", () => {
  it("should create an order with correct amount and currency", async () => {
    const order = await adapter.createOrderMock(9900, "INR", {
      userId: "u1",
      planId: "p1",
    });
    ...
    assert.ok(
      order.id.startsWith("order_test_"),
      "Order ID must have expected prefix",
    );
...
describe("Razorpay Adapter — Coupon Discount Math", () => {
  ...
  it("should floor percent discounts (no fractional paise)", () => {
    ...
    const price = 9999;
    const final = applyDiscount(
      { discountType: "percent", discountValue: 33 },
      price,
    );

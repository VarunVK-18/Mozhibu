 ...
 */
const computeMonthlyRevenue = async (
  year,
  month,
  adminUserId,
  force = false,
  ...
  if (adRevenue.isFinalized && !force) {
    throw new Error(
      `Revenue for ${year}-${month} is already finalized. Use force=true to recompute.`,
    );
  }
  ...
  if (!config) {
    config = {
      ...
      authorsPercent: 15,
      readersPercent: 5,
      minAuthorPayoutInPaise: 10000,
      minReaderPayoutInPaise: 5000,
    };
  }
  ...
  const bookAuthorMap = {};
  books.forEach((b) => {
    bookAuthorMap[b._id.toString()] = b.author.toString();
  });
  ...
  for (const row of qualifiedReadsByBook) {
    ...
    authorReadsMap[authorId] =
      (authorReadsMap[authorId] || 0) + row.qualifiedReads;
  ...
  for (const [authorId, reads] of Object.entries(authorReadsMap)) {
    const earnings =
      totalQualifiedReads > 0
        ? Number(
            (BigInt(reads) * BigInt(authorsPool)) / BigInt(totalQualifiedReads),
          )
        : 0;

    const status =
      earnings >= config.minAuthorPayoutInPaise ? "pending" : "rolled_over";
  ...
  const totalPlatformScore = engagementScores.reduce(
  ...
  for (const scoreDoc of engagementScores) {
    const reward =
      totalPlatformScore > 0
        ? Number(
            (BigInt(scoreDoc.totalScore) * BigInt(readersPool)) /
              BigInt(totalPlatformScore),
          )
        : 0;

    const status =
      reward >= config.minReaderPayoutInPaise ? "pending" : "rolled_over";

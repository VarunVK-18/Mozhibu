...
async function translateBooks(books, targetLang) {
    ...
    const toTranslate = books.filter(b => !b.titleTranslations || !b.titleTranslations.has(targetLang));
...
async function translateChapters(chapters, targetLang) {
    ...
    const titlesToTranslate = chapters.filter(c => !c.titleTranslations || !c.titleTranslations.has(targetLang));
...
${JSON.stringify(titlesObj)}`;
    ...
    const contentToTranslate = chapters.filter(c => !c.translations || !c.translations.has(targetLang));

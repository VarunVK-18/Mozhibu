...
router.get("/config", protect, superadmin, async (req, res) => {
  try {
    ...
    const engConfig = await EngagementScoreConfig.findOne().sort({
      createdAt: -1,
    });
...
router.put("/config", protect, superadmin, async (req, res) => {
  try {
    const {
      ...
      authorsPercent,
      readersPercent,
      minAuthorPayoutInPaise,
      minReaderPayoutInPaise,
    } = req.body;
...
router.put("/engagement-config", protect, superadmin, async (req, res) => {
  try {
    const config = new EngagementScoreConfig({
      ...req.body,
      updatedBy: req.user.id,
    });
    await config.save();
  ...
  try {
    ...
    if (existing && existing.isFinalized) {
      return res
        .status(400)
        .json({
          msg: "Revenue for this month is already finalized. Use force=true to override.",
        });
    }
...
router.get("/report/:year/:month", protect, superadmin, async (req, res) => {
  try {
    ...
    const totalAuthorEarnings = authorEarnings.reduce(
      (s, e) => s + e.earningsInPaise,
      0,
    );
    const totalReaderRewards = readerRewards.reduce(
      (s, r) => s + r.rewardInPaise,
      0,
    );
...
router.get("/analytics", protect, superadmin, async (req, res) => {
  try {
    ...
    const totalRevenues = await MonthlyAdRevenue.find()
      .sort({ year: -1, month: -1 })
      .limit(12);
...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      if (!earning)
        return res.status(404).json({ msg: "Earnings record not found" });

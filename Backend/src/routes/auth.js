...
const googleClient = new OAuth2Client(process.env.GOOGLE_CLIENT_ID || '1039454132466-7eo28db3tli7r28ckhhj822pmpi1k8sn.apps.googleusercontent.com');
  ...
  try {
    ...
    if (!authProvider || authProvider === 'normal') {
      ...
      if (!password || !passwordRegex.test(password)) {
        return res.status(400).json({ msg: 'Password must be 8-16 characters long, contain at least one uppercase letter and one number, and no emojis.' });
      }
    }
...
router.post('/google', async (req, res) => {
  try {
    ...
    const ticket = await googleClient.verifyIdToken({
      idToken: token,
      audience: process.env.GOOGLE_CLIENT_ID || '1039454132466-7eo28db3tli7r28ckhhj822pmpi1k8sn.apps.googleusercontent.com'
    });
    ...
    if (user) {
      ...
      return jwt.sign(payload, JWT_SECRET, { expiresIn: '5d' }, (err, token) => {
        if (err) throw err;
        const isProfileComplete = !!(user.mobile && user.preferredLanguage && user.mobile !== 'Not Provided');
        res.json({ token, isProfileComplete, user: { id: user.id, username: user.username, email: user.email, mobile: user.mobile, role: user.role, authorStatus: user.authorStatus, avatar: user.avatar } });
      });
    }
    ...
    return jwt.sign(payload, JWT_SECRET, { expiresIn: '5d' }, (err, token) => {
      if (err) throw err;
      res.json({ token, isProfileComplete: false, user: { id: user.id, username: user.username, email: user.email, mobile: user.mobile, role: user.role, authorStatus: user.authorStatus, avatar: user.avatar } });
    });
...
router.post('/forgot-password', async (req, res) => {
  try {
    ...
    if (user.authProvider !== 'normal') {
      return res.status(400).json({ msg: `You signed up with ${user.authProvider}, password reset is not applicable.` });
    }
    ...
    try {
      ...
      return res.status(500).json({ msg: 'Email could not be sent. Please check backend console for the link.' });
    }

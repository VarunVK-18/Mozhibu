...
router.post("/coupon/validate", protect, async (req, res) => {
  try {
    ...
    if (now < coupon.validFrom || now > coupon.validUntil) {
      return res
        .status(400)
        .json({ msg: "Coupon has expired or is not yet active" });
    }
    ...
    if (
      ...
      return res
        .status(400)
        .json({ msg: "Coupon not applicable to this plan" });
    }
...
router.post("/me/cancel", protect, async (req, res) => {
  try {
    ...
    await subscription.save();
    res.json({
      msg: "Auto-renewal cancelled. Your subscription remains active until expiry.",
    });
  } catch (err) {
    res.status(500).json({ msg: "Server Error" });
  }
